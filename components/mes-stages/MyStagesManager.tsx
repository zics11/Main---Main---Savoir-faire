  ...
  return (
    <div className="flex flex-col gap-2.5 border-t border-border bg-secondary/40 p-4">
      {stage.dates.map((date) => (
        ...
        >
          ...
          <div className="flex items-center gap-4">
            ...
            <button
              type="submit"
              formAction={deleteMyStageDate.bind(null, date.id)}
      ...
      <details className="rounded-sm border border-dashed border-input p-3">
        ...
        <form
          action={createMyStageDate.bind(null, stage.id)}
...
export function MyStagesManager({
  stages,
}: {
  ...
  return (
    <section className="rounded-sm border border-border bg-card p-6">
      <h2 className="mb-1 font-serif text-xl font-semibold">Mes stages</h2>
      ...
      <div className="flex flex-col gap-4">
        {stages.map((s) => (
          <div key={s.id} className="overflow-hidden rounded-sm border border-border">
            <form action={updateMyStage.bind(null, s.id)} className="p-4">
              <StageFields stage={s} />
              <div className="mt-3 flex items-center gap-4">
                ...
                <button
                  type="submit"
                  formAction={deleteMyStage.bind(null, s.id)}
        ...
        {stages.length === 0 && (
          <p className="text-sm text-muted-foreground">
            Vous n&apos;avez encore aucun stage publié.
          </p>
        )}
      </div>
      ...
      <details className="mt-5 rounded-sm border border-dashed border-input p-4">
        ...
        <form action={createMyStage} className="mt-4">
          <StageFields />
          <button
            type="submit"
